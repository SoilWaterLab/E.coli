...
#Keiran: "C://Users//Ross//Desktop//R_code//Ecoli_project//liquid_growth_curve//"
#Allison: "C://Users//Owner//Dropbox//Cornell//Ecoli_research//Data//"
dir <- "C://Users//Owner//Dropbox//Cornell//Ecoli_research//Data//Growth Curve//liquid_growth_curve//"
setwd(dir)

#Open files
curve_list <- data.frame(list.files(path=dir, pattern="*hr"))
curve_list<-curve_list[c(13,14,4,8,9,10,11,12,1,2,3,5,6,7),]

i=1
while (i<((length(curve_list))+1)){
  filename <- paste("data_",i,sep="")
  f <- read.csv(paste(dir,curve_list[i],sep=""))
  #assign(filename,f)
  LB0.05 <- f[2:9,2:13]
  LB0.1 <- f[13:20,2:13]
  LB1 <- f[24:31,2:13]
...
LB1_list <- as.data.frame(LB1_list[c(1,7:14,2:6),])
colnames(LB1_list) <- "File"
LB0.1_list <- data.frame(list.files(path=dir, pattern="*_LB0.1.csv*"))
LB0.1_list <- as.data.frame(LB0.1_list[c(1,7:14,2:6),])
colnames(LB0.1_list) <- "File"
LB0.05_list <- data.frame(list.files(path=dir, pattern="*_LB0.05.csv*"))
LB0.05_list <- as.data.frame(LB0.05_list[c(1,7:14,2:6),])
colnames(LB0.05_list) <- "File"

# Thi is totally a brute force way to do this, but I am going to make the three different plots (lx LB, 0.1x LB, and
# 0.05x LB) by just copying and pasting the code for the 1x LB - Allison
...
CR_growth_plot <- ggplot(all, aes(x=Time, y=y,colour=Treatment, group=Treatment, shape=Treatment))+
  ...
  scale_color_manual(values=c("#1f78b4","#a6cee3","Red","#33a02c","#b2df8a","#fdae61","grey"))+
  ylab("OD")+
  xlab("Time (hr)")+
  theme_bw()+
  theme(axis.text=element_text(size=40), axis.title.x=element_text(size=40,face="bold",vjust=-0.4), 
        axis.title.y=element_text(size=40,vjust=1.2), legend.text=element_text(size=40),
        legend.title=element_text(size=40,face="bold"), legend.position=c(0.2,0.5))
CR_growth_plot
...
# 0.1x LB --------------------
while (i<((nrow(LB0.1_list))+1)){
  potato <- read.csv(paste(dir,LB0.1_list$File[i],sep=""))
  red <- potato[1,1:3]
  red <- as.data.frame(rollapply(as.numeric(red), 3, mean, by = 3, partial = FALSE))
  colnames(red) <- paste("Time",i,sep="")
  white <- potato[1,4:6]
...
png(filename=paste(dir,"liquid_curve_0.01xLB.png",sep=""), height=1200, width=1600)
plot(CR_growth_plot)
dev.off()

# 0.05x LB ------------------------------------
while (i<((nrow(LB0.05_list))+1)){
  potato <- read.csv(paste(dir,LB0.05_list$File[i],sep=""))
  red <- potato[1,1:3]
  red <- as.data.frame(rollapply(as.numeric(red), 3, mean, by = 3, partial = FALSE))
  colnames(red) <- paste("Time",i,sep="")
  white <- potato[1,4:6]
...
png(filename=paste(dir,"liquid_curve_0.05xLB.png",sep=""), height=1200, width=1600)
plot(CR_growth_plot)
dev.off()
